...
resit_nonlinear_t1 = pd.read_csv('~/Documents/M3R/CD/tests/resit_nonlinear_t1.csv')
resit_nonlinear_t2 = pd.read_csv('~/Documents/M3R/CD/tests/resit_nonlinear_t2.csv')

gcm_1 = np.array(gcm_nonlinear_t1.iloc[:,1])
gcm_2 = np.array(gcm_nonlinear_t2.iloc[:,1])
kci_1 = np.array(kci_nonlinear_t1.iloc[:,1])
kci_2 = np.array(kci_nonlinear_t2.iloc[:,1])
res_1 = np.array(resit_nonlinear_t1.iloc[:,1:])
res_2 = np.array(resit_nonlinear_t2.iloc[:,1:])

gcm_nonlinear_t1 = gcm_nonlinear_t1.drop(columns='Unnamed: 0')
kci_nonlinear_t1 = kci_nonlinear_t1.drop(columns='Unnamed: 0')
resit_nonlinear_t1 = resit_nonlinear_t1.drop(columns='Unnamed: 0')
gcm_nonlinear_t1 = gcm_nonlinear_t1.rename(columns={'t1':'GCM'})
kci_nonlinear_t1 = kci_nonlinear_t1.rename(columns={'t1':'KCI'})
resit_nonlinear_t1 = resit_nonlinear_t1.rename(columns={'0':'RESIT'})
t1_data = pd.concat([gcm_nonlinear_t1,kci_nonlinear_t1,resit_nonlinear_t1],axis=1)
t1_data = t1_data.rename(index={0:'n = 50',1:'n = 100',2:'n = 150',3:'n = 200'})
ax = plt.axes()
